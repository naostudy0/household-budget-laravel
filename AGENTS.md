...
superpowers によるTDDで最初にテストを書く場合でも、常に Feature テストへ詳細ケースを追加するわけではない。

HTTPルートに対する既存のテストがない場合は、そのルートの基本的な振る舞いを Feature テストで書いてよい。

HTTPルートに対する既存の Feature テストがある場合は、追加しようとしている挙動が「一般的なユーザー体験として確認すべきもの」かを判断する。

以下に当てはまるものは Feature テストへ追加してよい。

- 主要な正常系フロー
- ユーザーが通常操作で遭遇しうる代表的な失敗
- 画面遷移、リダイレクト、Inertiaページ名、主要propsなど、HTTP境界で保証したい振る舞い

以下に当てはまるものは、原則として Feature テストへ追加しない。

- 稀なエッジケース
- 入力値の組み合わせを網羅するためだけのケース
- HTTP境界ではなく、UseCase / Domain / Validation の詳細として確認すべきケース

これらは必要に応じて Unit または Integration テストに寄せる。

バリデーションのテストは、Feature テストだけで済ませない。

Feature テストでは、ユーザー体験として重要な代表ケースを確認する。

Integration テストでは、バリデーションルールの全パターンを網羅する。
